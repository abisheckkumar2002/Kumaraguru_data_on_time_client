...
import { getindcontent, updateindcontent } from "../../actions/users";
...
export default function UserProfile(props) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    console.log(reqData);
    let { error } = await updateindcontent(reqData,id);
    console.log("error", error);
    if (isEmpty(error)) {
      toast.success("Updated Successfully", toasterOption);
      history("/indcontentlist");
    } else {
  ...
  const getUserData = async () => {
    var test = await getindcontent(id);
    console.log(test)
    let data = {};
          ...
          <Card>
            ...
            >
              <CardHeader color="primary">
                <h4 className={classes.cardTitleWhite}>Edit Resources industrial content</h4>
                <p className={classes.cardCategoryWhite}>update the industrial content</p>
              </CardHeader>
              <CardBody>
              <GridContainer>
