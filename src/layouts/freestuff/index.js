...
import { getfreestufflist,deletefreestuff } from "../../actions/users";
...
function Item() {
  ...
  const add =()=>{
    window.location = "/admin/freestuffadd"
  }
  const video =(id)=>{
    if (id != "") {
    window.location = "/admin/freestuffmultiple/"+id
    }
  }
  const pdf =(id)=>{
    if (id != "") {
    window.location = "/admin/freestuffmultiplepdf/"+id
    }
  }
  const folder =(id)=>{
    if (id != "") {
  ...
  function viewR(id) {
    if (id != "") {
      window.location ="/admin/freestuffview/"+id;
    }
  }

  function editR(id) {
    if (id != "") {
      window.location ="/admin/freestuffedit/"+id;
    }
  }
  ...
  const getUserListdata = async () => {
    ...
    var test = await getfreestufflist();
    const mapped = await Promise.all(test.userValue.map(element =>{ Object.assign(element, {
      action: (
       ...
       <Button  className="ml-3"
        ...
        {/* <MDTypography component="a"  onClick={() => folder(element._id)}  variant="caption" color="text" fontWeight="medium">
       <Button  className="ml-3"
            variant="contained"
