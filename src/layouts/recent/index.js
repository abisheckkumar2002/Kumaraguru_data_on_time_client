...
import { getrecentlist,deletestudy } from "../../actions/users";
...
function Item() {
  ...
  const columns= [
    { 
      Header: "NEET LIVE CLASSES", 
      accessor: "image", 
      align: "left" 
    },
    { 
      Header: "JEE LIVE CLASSES", 
      accessor: "image2", 
      align: "left" 
    },
    // { 
  ...
  function deleteR(id) {
    if (id != "") {
      ...
      toast.success("deleted success");
       history("/studyindex");
      window.location = "/recentindex";
    }
  }

  function editR(id) {
    if (id != "") {
      window.location ="/admin/recentedit/"+id;
    }
  }
  function viewR(id) {
    if (id != "") {
      window.location ="/admin/recentview/"+id;
    }
  }

  const getUserListdata = async () => {
    var test = await getrecentlist();
    ...
    element.image2= <img src={config.API + "/images/user/" + element.image2} alt="..." style={{maxWidth:200, maxHeight:200}} />;
return element;
