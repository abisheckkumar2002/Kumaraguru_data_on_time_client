...
export default function UserProfile(props) {
  ...
  const handleProcedureContentChange = (contentsatta, delta, source, editor) => {
    ...
    let formData = { ...formValue, ...{ ["question"]: contentsatta } };
  ...
  const handleProcedureContentChange1 = (contentsatta, delta, source, editor) => {
    //let has_attribues = delta.ops[1].attributes || "";
    console.log(contentsatta);
    let formData = { ...formValue, ...{ ["answer"]: contentsatta } };
    setFormValue(formData);
    //const cursorPosition = e.quill.getSelection().index;
    // this.quill.insertText(cursorPosition, "★");
    //this.quill.setSelection(cursorPosition + 1);
  ...
  const handleFormSubmit = async (e) => {
    ...
    console.log(reqData);
    let { error } = await updatefaq(reqData,id);
  ...
  const getUserData = async () => {
    ...
    data["question"] = removeTags(test.userValue.question);
    data["answer"] =removeTags (test.userValue.answer);
