...
import { upcomingSchedules,completedSchedules,deletezoommeet } from "../../actions/users";
...
function Item() {
  ...
  function scheduleeditR(id) {
    if (id != "") {
      window.location ="/admin/scheduleeditmaterial/"+id;
    }
  }
...
async function deleteR(id,b_id) {
  ...
  const getUserListdata = async () => {
    var test = await upcomingSchedules(id,b_id);
    ...
    const mapped = await Promise.all(test.userValue.map(element => Object.assign(element, {
      ...
      action: (
       ...
       <Button  className="ml-3"
        ...
        <MDTypography component="a"  onClick={() => scheduleeditR(element._id)}  variant="caption" color="text" fontWeight="medium">
       <Button  className="ml-3"
            variant="contained"
            color="primary">Add materials</Button>
        </MDTypography>
      ...
      image: <img src={config.API + "/images/user/" + element.image} alt="..." style={{maxWidth:100, maxHeight:100}} />,
      start_time:    <Moment format="DD/MM/YYYY hh:mm A">
     {element.start_time}
  </Moment>,
        end_time:    <Moment format="DD/MM/YYYY hh:mm A">
        {element.end_time}
     </Moment>,
    })));
    setUserDet(test.userValue);

    var test = await completedSchedules(id,b_id);
  ...
  function add() {
    ...
    if (id != "") {
      window.location ="/admin/couresscheduleadd/"+id+"/"+b_id+"/"+type;
    }
  }
