...
import { getlivecourselist,deletelivecourse } from "../../actions/users";
...
function Item() {
  ...
  const add =()=>{
    window.location = "/admin/livecourseadd"
  }
  const video =(id)=>{
    if (id != "") {
    window.location = "/admin/livecoursemultiple/"+id
    }
  }
  const pdf =(id)=>{
    if (id != "") {
    window.location = "/admin/livecoursemultiplepdf/"+id
    }
  }
  ...
  function Batch(id) {
    if (id != "") {
      window.location ="/admin/course-batches-index/"+id+"/62cd0ca31538ec3c90ab3c73";
    }
  }
  ...
  const getUserListdata = async () => {
    var test = await getlivecourselist();
    ...
    element.image= <img src={config.API + "/images/user/" + element.image} alt="..." style={{maxWidth:70, maxHeight:70}} />;
return element;
