...
import { addsubpdf,addfile,getsubchapterpdfdata,deletefile,getsubjectlist,getexamlist,getclasslist,gettopiclist,getfreelist } from "actions/users";
...
export default function UserProfile() {
  ...
  function checktype() {
    ...
    if (!inputList[0].file) {
        alert('Please import only pdf file');
  ...
  const handleFormSubmit = async (e) => {
    ...
    let { error } = await addsubpdf(reqData);
...
const userData = async() =>{
  ...
  const listdata=await getsubchapterpdfdata(id1);
  const test3=await gettopiclist(id);
  const test4 = await getfreelist();
  console.log(listdata,"sss");
...
<GridItem xs={12} sm={12} md={4}>
                    <h6 style={{ color:"blue" }}>Note:1.Free pdf demo will be applicable only for app under studymaterials options</h6>
