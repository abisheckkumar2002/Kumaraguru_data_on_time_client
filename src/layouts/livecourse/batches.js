...
import { getlivecoursebatcheslist,deletebatch } from "../../actions/users";
...
function Item() {
  ...
  async function deleteR(id) {
    const result = await confirm("Are you sure do you want to delete the batch,if the course not completed for the batch,please don't delete?");
    if (result) {
      if (id != "") {
  ...
  function schedule(course_id,batch_id) {
  ...
  const getUserListdata = async () => {
    var test = await getlivecoursebatcheslist(id);
      ...
      action: (
        <>
            ...
            color="primary">Students Mangement</Button>
               ...
               <MDTypography component="a"   onClick={() => schedule(id,element._id)} variant="caption" color="text" fontWeight="medium">
              <Button className="ml-3"
            variant="contained"
