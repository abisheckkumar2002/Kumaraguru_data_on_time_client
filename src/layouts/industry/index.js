...
import { getindustrylist,deleteindustry } from "../../actions/users";
...
function Item() {
  ...
  const add =()=>{
    window.location = "/industryadd"
  }

  function editR(id) {
    if (id != "") {
      window.location ="/industryedit/"+id;
    }
  }
  function viewR(id) {
    if (id != "") {
      window.location ="/industryview/"+id;
    }
  }
  ...
  function deleteR(id) {
    if (id != "") {
      ...
      window.location = "/industryindex";
    }
  }
  const getUserListdata = async () => {
    
    var test = await getindustrylist();
...
return element;
    ...
    // await Promise.all(test.userValue.map(e=> e.action=));
    console.log(mapped, "saran123");
